...
def create_cytoscape_network(results, network_title="Protein_Interaction_Network", run_output_path=".", nodes_data=None):
    ...
    # Nodes aufbauen – auch wenn es keine Edges gibt (Single-Chain)
    if nodes_data:
    ...
    # Zwei Varianten vorbereiten:
    # - Für .cyjs-Export: leerer DF ist ok
    # - Für Cytoscape: None statt leerem DF übergeben
    if edges:
        ...
        edges_df_for_cyto = None  # <- entscheidend, um den CyError zu vermeiden

    # Pfad 1: kein Cytoscape -> direkt .cyjs (Nodes-only möglich)
    if not config.get("open_in_cytoscape", True):
        export_custom_cyjs(nodes_df, edges_df_for_export, network_title, run_output_path)
        return

    # Pfad 2: Cytoscape nutzen (auch Nodes-only möglich)
    try:
        p4c.cytoscape_ping()
    ...
    # WICHTIG: edges_df_for_cyto ist None, wenn keine Edges vorhanden sind
